...
class MesaData:
    ...
    def read_data(self):
        ...
        if self.file_type == 'lines':
            self.read_log_data()
        elif self.file_type == 'np_binary':
            self.read_log_data()
        else:
    ...
    def read_log_data(self):
        ...
        None
        """
        if self.file_type == 'np_binary':
            self.bulk_data = self.file_name[1]
            ...
            header_data = []
            if self.file_type == 'lines':
                for i, line in enumerate(self.file_name):
    ...
    def read_model_data(self):
        ...
        while not found_blank_line:
            ...
            i += 1
            found_blank_line = (blank_line_matcher.match(lines[i]) is not None)
        temp_data = np.array(temp_data, dtype=object).T
